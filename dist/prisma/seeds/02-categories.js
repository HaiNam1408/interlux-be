...
const createSlug_util_1 = require("./../../src/utils/createSlug.util");
async function seedCategories(prisma) {
    ...
    for (const category of mainCategories) {
        await prisma.category.create({
            data: {
                name: category.name,
                slug: createSlug_util_1.SlugUtil.createSlug(category.name),
                sort: category.sort,
                status: 'ACTIVE',
            },
        });
        ...
        for (const subCategory of livingRoomSubCategories) {
            await prisma.category.create({
                data: {
                    ...
                    slug: createSlug_util_1.SlugUtil.createSlug(subCategory.name),
